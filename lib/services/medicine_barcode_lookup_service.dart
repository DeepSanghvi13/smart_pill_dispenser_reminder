...
class MedicineBarcodeLookupService {
  ...
  static Future<BarcodeLookupResult?> lookupByBarcode(
    ...
    if (cacheReader != null) {
      for (final candidate in _barcodeCandidates(digits)) {
        ...
        if (cached != null) {
          ...
          final categoryName =
              (cached['category'] as String? ?? 'tablets').trim();
          final expiryRaw = cached['expiryDate'] as String?;
          final healthCondition =
              (cached['healthCondition'] as String?)?.trim();
          final expiryDate =
    ...
    final backendResult =
        await (backendLookup ?? _lookupFromBackendProxy)(digits);
  ...
  static Future<BarcodeLookupResult?> _lookupFromBackendProxy(
      String digits) async {
    try {
      ...
      return BarcodeLookupResult(
        ...
        dosage: dosage,
        category: MedicineCategory.fromString(categoryName),
        expiryDate: (data['expiryDate'] is String)
            ? DateTime.tryParse(data['expiryDate'] as String)
            : null,
        healthCondition: (data['healthCondition'] as String?)?.trim(),
  ...
  static List<String> _ndcCandidates(String digits) {
    ...
    if (digits.length == 11) {
      ndc.add(
          '${digits.substring(0, 5)}-${digits.substring(5, 9)}-${digits.substring(9, 11)}');
    }
    ...
    if (digits.length >= 10) {
      ...
      ndc.add(
          '${tail10.substring(0, 4)}-${tail10.substring(4, 8)}-${tail10.substring(8, 10)}');
      ndc.add(
          '${tail10.substring(0, 5)}-${tail10.substring(5, 8)}-${tail10.substring(8, 10)}');
      ndc.add(
          '${tail10.substring(0, 5)}-${tail10.substring(5, 9)}-${tail10.substring(9, 10)}');
    }
  ...
  static Future<BarcodeLookupResult?> _lookupFromOpenFda(
    ...
    try {
      for (final ndc in _ndcCandidates(digits)) {
        ...
        final response =
            await requestClient.get(uri).timeout(const Duration(seconds: 8));
        ...
        if (activeIngredients is List && activeIngredients.isNotEmpty) {
          ...
          if (ingredient is Map<String, dynamic>) {
            dosage =
                (ingredient['strength'] as String?)?.trim().isNotEmpty == true
        ...
        final dosageForm =
            (first['dosage_form'] as String? ?? '').toLowerCase();
