  ...
  Future<void> _retryPendingSync() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          success
              ? 'Pending database sync completed.'
              : 'Retry failed. MongoDB API is still unreachable.',
        ),
      ),
    );
          ...
          children: [
            ...
            SqlStatusCard(
              ...
              subtitle: _pendingSync
                  ? 'Some local changes are waiting for database sync.'
                  : 'No pending database sync tasks.',
              color: pendingColor,
            ...
            _buildCategoryPageTile(
              ...
              rows: _users,
              columns: const ['id', 'email', 'isCaretaker', 'createdAt'],
            ),
            _buildCategoryPageTile(
